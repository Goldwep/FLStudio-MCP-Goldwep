...
> **Status:** Currently at **v1.0.0-rc2** (file-IPC transport). All code work is done. Single user action gates v1.0.0 final.
...
- IPC folder pre-created (`Settings\Hardware\FLStudio-MCP\ipc\`) — required because `os.mkdir` is broken in FL's embedded Python

Do this:

1. **In FL Studio:** `F10` → MIDI Settings → pick an input port → Controller type = `"FLStudio MCP Bridge"` → click Enable.
2. **Confirm:** Script Output window (Ctrl+F12) shows a new tab `"FLStudio MCP Bridge"` with `[mcp-bridge] MCP file-IPC bridge ready at ...\FLStudio-MCP\ipc`. A heartbeat file `bridge_alive.txt` appears in the `ipc/` folder.
3. **From repo root:**
...
- Polls the IPC folder every 1s for up to 60s, waiting for `bridge_alive.txt`.
- Probes all 88 dispatch entries via the production FileBridge.
- Categorizes results per method: ok / fail / timeout / skip.
- Writes a full report to `docs/LIVE-VERIFY-REPORT.md`.
- For each `AttributeError` failure, suggests the vendor-script-confirmed alternative from `bridge-contract-audit.md`.

**Exit codes:**

- `0` — all green, ready for v1.0.0
- `1` — one or more failures (almost certainly the `[UNVERIFIED]` tools; apply the suggested renames)
- `2` — bridge unreachable (heartbeat file never appeared; see step 1, and confirm the `ipc/` folder exists)
...
git add -A
git commit -m "v1.0.0 — live-verified against FL Studio (file-IPC transport)"
git tag -a v1.0.0 -m "v1.0.0 final — file-IPC bridge live, 88 dispatch entries verified"
git push origin main
git push origin v1.0.0

gh release create v1.0.0 --repo Goldwep/FLStudio-MCP-Goldwep \
  --title "v1.0.0 — FL Studio MCP (file-IPC)" \
...
Read `docs/LIVE-VERIFY-REPORT.md`. For each failure, the suggested action will be one of:
...
- Bridge code paths (88/88 entries) — exercised via `tests/bridge_device_smoke.py` against mocked FL modules + a temp IPC folder.
- FileBridge — 6 unit tests (connect-fail / ok / error / timeout / out-of-order / transient-partial).
- Smoke test — `npm test` (4 smoke + 6 file-bridge = 10/10 pass).
- Build, lint, format — all clean.

The mock smoke exercises the same dispatch + IPC framing + error code paths as live FL — what `verify:live` adds is verification that **the FL modules actually expose the methods we expect** for the `[UNVERIFIED]` 10.
